...
public class WhiteLineRenderer {
    private static final WhiteLineRenderer INSTANCE = new WhiteLineRenderer();
    ...
    public static WhiteLineRenderer getInstance() {
        return INSTANCE;
    }
    ...
    public void addWave(Vec3d pos, float range, float speed) {
        for (int i = 0; i < 150; i++) {
            if (!wavs.containsKey(i) || wavs.get(i) == null) {
                ...
                wave.startWave();
                WhiteLineShader.getInstance().setCenter(i, pos);
                WhiteLineShader.getInstance().setMaxRadiuss(i, range);
                wavs.put(i, wave);
                return;
            }
        }
    ...
    private void render(Matrix4f viewMatrix, Matrix4f projectionMatrix) {
        ...
        dl.forEach(n -> {
            WhiteLineShader.getInstance().removeCenter(n);
            WhiteLineShader.getInstance().removeMaxRadiuss(n);
            wavs.remove(n);
        });

        WhiteLineShader.getInstance().setArrayCenter();
        WhiteLineShader.getInstance().setArrayMaxRadiuss();
        ...
        WhiteLineShader.getInstance().setInverseViewMatrix(invertedViewMatrix);

        Matrix4f invertedProjectionMatrix = new Matrix4f(projectionMatrix);
        invertedProjectionMatrix.invert();
        WhiteLineShader.getInstance().setInverseProjectionMatrix(invertedProjectionMatrix);

        Vec3d position = mc.gameRenderer.getActiveRenderInfo().getProjectedView();
        WhiteLineShader.getInstance().setPosition(position);

        wavs.forEach((m, n) -> WhiteLineShader.getInstance().setRadiuss(m, n.getRadius()));

        WhiteLineShader.getInstance().setArrayRadiuss();

        RESET_BLEND_STATE.apply();

        WhiteLineShader.getInstance().bind();

        blit(framebuffer);

        WhiteLineShader.getInstance().unbind();
    }
    ...
    public void deleteDepthCopyFramebuffer() {
        WhiteLineShader.getInstance().setDepthBuffer(0);
    ...
    private void createDepthCopyFramebuffer() {
        ...
        WhiteLineShader.getInstance().setDepthBuffer(depthCopyDepthBuffer);
    }
